...
describe("Foreign Keys", () => {
    ...
    it("should not be able to delete category if any app is linked", async done => {
        const categoryId = 6;
        const query = `SELECT COUNT(*) AS appCount FROM ${CATEGORIES} WHERE category_id = ${categoryId}`;
        try {
    ...
    it("should not be able to delete pricing plan if any app is linked", async done => {
        ...
        const query = `SELECT COUNT(*) AS appCount FROM ${PRICING_PLANS} WHERE pricing_plan_id = ${pricingPlanId}`;;
    ...
    it("should not be able to delete app if any data is linked", async done => {
        const appId = 245;
        const query = `SELECT COUNT(*) AS linkedDataCount FROM ${APPS} WHERE app_id = ${appId}`;;
